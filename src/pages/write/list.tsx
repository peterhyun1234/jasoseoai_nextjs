...
const WriteList = () => {
  ...
  const getResumeList = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.get(`/writtenResumes?userId=${userId}`, {
        headers: {
    ...
    <>
      ...
      <WrapBox>
        {user !== undefined && user !== null && user.username !== undefined && (
          <ResumeListTitle>
            <ResumeListTitleSpan>{user.username}</ResumeListTitleSpan>님이 작성한 자기소개서
          </ResumeListTitle>
        )}
        <ResumeListDiv>
          ...
          {resumeList?.map((resume: any) => (
            <ResumeListItem
              key={resume.id}
              onClick={() => {
                router.push(`/write/detail/${resume.id}`);
              }}
            >
              <ResumeListItemTitle>{`🧾 ${resume.company}`}</ResumeListItemTitle>
              <ResumeListItemContent>{resume.job}</ResumeListItemContent>
              <ResumeListItemDate>
